...
pub fn build_transport(
	...
	let transport = if !memory_only {
		...
		let tcp_trans = tcp::TcpTransport::new(tcp_config.clone());
		let dns_init = futures::executor::block_on(dns::DnsConfig::system(tcp_trans));
		...
		EitherTransport::Left(if let Ok(dns) = dns_init {
			...
			let tcp_trans = tcp::TcpTransport::new(tcp_config);
			let dns_for_wss = futures::executor::block_on(dns::DnsConfig::system(tcp_trans))
